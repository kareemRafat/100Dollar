...
- [x] **T2.2** Fix `sendOtp` race: Change `updateOrCreate` to check if a verified vote exists *first* and return early. This prevents resetting `otp_verified_at` for an existing vote.
...
- [ ] **T5.2** Update `HandleInertiaRequests` shared props to reflect the new key format for block status
